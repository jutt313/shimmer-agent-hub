...
```Okay, you've provided the current `AutomationDetail.tsx` file. Now, let's update this file to integrate the `AIAgentForm` correctly and allow the AI to be aware of newly saved agents.

Here's the full updated code for `src/pages/AutomationDetail.tsx`. Please copy and paste this entire block to replace the contents of your file:
...
const AutomationDetail = () => {
  ...
  const handleSendMessage = async (messageText: string) => { // Modified to accept messageText
    if (!messageText.trim() || sendingMessage || !automation) return; // Use messageText
    ...
    setMessages(prev => [...prev, userMessage]);
    setNewMessage(""); // Clear input only if it was from the input field
    setSendingMessage(true);
    ...
    try {
      ...
      await supabase
        ...
        .insert({
          automation_id: automation.id,
          sender: 'user',
          message_content: messageText // Use messageText
        });

      // Get AI response
      const { data, error } = await supabase.functions.invoke('chat-ai', {
        body: { 
          message: `In the context of automation "${automation.title}": ${messageText}`, // Use messageText
          messages: messages.slice(-10)
        }
      });
  ...
  const handleKeyPress = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && !sendingMessage) {
      handleSendMessage(newMessage); // Pass newMessage from input
      setNewMessage(""); // Clear input after sending
    }
  };

  // NEW: handleAgentSaved function to be passed to AIAgentForm
  const handleAgentSaved = (agentName: string, agentId: string) => {
    setShowAIAgentForm(false); // Close the form
    if (automation) { // Ensure automation is not null
      ...
      setMessages(prev => [...prev, {
        ...
        isBot: false, // This is technically the user's action
        timestamp: new Date()
      }]);

      // Then send a new prompt to AI to update the blueprint/conversation
      handleSendMessage(`Please incorporate the newly configured AI Agent "${agentName}" (ID: ${agentId}) into this automation's blueprint and explain its role and impact on the workflow.`).then(() => {
        // Clear input field if it was used for the last message
        setNewMessage("");
      });
    }
  };
  ...
  const handleAgentAdd = (agent: any) => {
    // This function is currently unused in the context of opening the form,
    // as the AI Agent button directly calls setShowAIAgentForm(true).
    // If you intend to pre-fill the form, this is where that logic would go.
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 p-6 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto h-full flex flex-col relative z-10 pt-16">        
        ...
        <div className="space-y-4 pb-6">
          <div className="flex gap-4 items-end px-[60px]">
            ...
            <Button 
              onClick={() => handleSendMessage(newMessage)} // Changed to call handleSendMessage directly with newMessage
              disabled={sendingMessage || !newMessage.trim()}
              className="rounded-3xl bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white px-8 py-4 shadow-lg hover:shadow-xl transition-all duration-300 border-0 disabled:opacity-50" 
              style={{
                boxShadow: '0 0 30px rgba(92, 142, 246, 0.3)'
      ...
      {/* AI Agent Form Modal */}
      {showAIAgentForm && automation && ( // Ensure automation is not null before rendering
        <AIAgentForm
          automationId={automation.id} // Pass the current automation ID
          onClose={() => setShowAIAgentForm(false)}
          onAgentSaved={handleAgentSaved} // Pass the new callback
        />
      )}
    </div>
  );
