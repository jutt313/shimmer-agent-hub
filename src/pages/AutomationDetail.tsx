...
const AutomationDetail = () => {
  ...
  const handleAgentSaved = async (agentName: string, agentId?: string, llmProvider?: string, model?: string, config?: any, apiKey?: string) => {
    ...
    // Create structured message for ChatAI to generate dynamic response
    const isCustomAgent = !config?.fromChatAI;
    const agentType = isCustomAgent ? "custom" : "ChatAI-recommended";
    
    const structuredAgentData = {
      action: "agent_added",
      ...
      goal: config?.goal || 'Not specified', 
      rules: config?.rules || 'Not specified',
      memory: config?.memory || 'Not specified',
      from_chatai_recommendation: config?.fromChatAI || false
    };

    const messageToAI = `I just added a new AI agent to my automation. Here are the complete details:
...
Please explain in detail how this agent will work in my automation, what benefits it provides, how it integrates with my workflow, and provide a step-by-step explanation of how this agent will enhance my automation process.`;

    // Send to ChatAI for dynamic response
    await handleSendMessage(messageToAI);
    
    // Success feedback
    toast({
      title: "Agent Saved",
      description: `AI Agent "${agentName}" has been successfully configured and added to your automation.`,
    });
  ...
  const handleAgentDismiss = async (agentName: string) => {
    ...
    // Create structured message for ChatAI to generate dynamic dismissal response
    const messageToAI = `I have dismissed the AI agent recommendation for "${agentName}". 

Please:
1. Acknowledge that I dismissed this agent recommendation
2. Confirm you won't recommend this specific agent again for this automation
3. Provide a summary of how my automation will work WITHOUT this agent
4. Explain any alternative approaches or different agent types you might suggest instead
5. Give me a step-by-step overview of my current automation workflow without this dismissed agent

I want to understand the full impact of dismissing this agent and what my automation capabilities are without it.`;
    ...
    toast({
      ...
      description: `Agent "${agentName}" has been dismissed and will not be recommended again.`,
      variant: "destructive",
    });
  };
