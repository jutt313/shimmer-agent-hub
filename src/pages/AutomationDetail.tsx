...
const AutomationDetail = () => {
  ...
  const generateChatMessageForAgentSave = (agentName: string, agentConfig: any) => {
    const isCustomAgent = !agentConfig.fromChatAI;
    
    if (isCustomAgent) {
      return `🤖 **Custom AI Agent Added Successfully!**

**Agent Name:** ${agentName}
**LLM Provider:** ${agentConfig.llmProvider || 'OpenAI'}
**Model:** ${agentConfig.model || 'GPT-4'}

**Configuration:**
- **Role:** ${agentConfig.role || 'Not specified'}
- **Goal:** ${agentConfig.goal || 'Not specified'}
- **Rules:** ${agentConfig.rules || 'Not specified'}
- **Memory:** ${agentConfig.memory || 'Not specified'}

**How this agent will work in your automation:**
1. **Intelligent Decision Making:** This agent will analyze incoming data and make smart decisions based on its role and rules
2. **Context Awareness:** It will remember important information using its memory configuration
3. **Goal-Oriented Actions:** Every action will be aligned with the specified goal
4. **Rule Compliance:** All decisions will follow the defined rules and constraints

**Integration Benefits:**
✅ **Enhanced Automation Logic** - Your agent will handle complex scenarios automatically
✅ **Adaptive Responses** - It will adjust behavior based on different inputs and conditions
✅ **Continuous Learning** - The agent will improve its responses over time
✅ **Reliable Execution** - Consistent performance following your defined parameters

Your custom agent is now ready to be integrated into automation workflows and will help make your processes more intelligent and adaptive!`;
    } else {
      return `🤖 **ChatAI Recommended Agent Added!**

**Agent Name:** ${agentName}
**Source:** Generated by ChatAI Analysis
**LLM Provider:** ${agentConfig.llmProvider || 'OpenAI'}
**Model:** ${agentConfig.model || 'GPT-4'}

**Agent Specialization:**
- **Role:** ${agentConfig.role || 'Specialized automation assistant'}
- **Goal:** ${agentConfig.goal || 'Optimize automation performance'}
- **Rules:** ${agentConfig.rules || 'Follow best practices and user preferences'}
- **Memory:** ${agentConfig.memory || 'Context-aware decision making'}

**How this ChatAI agent will enhance your automation:**
1. **AI-Optimized Performance:** This agent was specifically recommended based on your automation requirements
2. **Smart Context Understanding:** It understands your automation's purpose and adapts accordingly
3. **Predictive Actions:** The agent can anticipate needs and take proactive steps
4. **Error Prevention:** Built-in intelligence to avoid common automation pitfalls

**Automation Integration:**
✅ **Seamless Workflow Integration** - Designed to work perfectly with your specific automation
✅ **Intelligent Monitoring** - Continuously watches for optimization opportunities
✅ **Dynamic Adaptation** - Adjusts behavior based on real-time conditions
✅ **Performance Enhancement** - Improves automation efficiency and reliability

This AI-recommended agent is now configured and ready to make your automation smarter and more effective!`;
    }
  };

  const generateChatMessageForAgentDismissal = (agentName: string) => {
    return `❌ **Agent Recommendation Dismissed**

**Agent Name:** ${agentName}

You have dismissed this AI agent recommendation. I understand this agent doesn't fit your current automation needs.

**What this means:**
- This specific agent will not be recommended again for this automation
- I'll remember your preference and avoid suggesting similar configurations
- Your automation will continue without this agent's functionality

**Alternative Options:**
- You can still create custom agents with different configurations
- I can suggest different types of agents if needed
- You can always add agents manually through the AI Agent form

**Note:** If you change your mind later, you can always create a similar agent manually with custom settings that better match your requirements.

I'll focus on other optimization suggestions that better align with your automation goals.`;
  };
  ...
  const handleAgentSaved = async (agentName: string, agentId?: string, llmProvider?: string, model?: string, config?: any, apiKey?: string) => {
    ...
    // Generate and send chat message explaining the agent
    const agentConfig = {
      ...config,
      llmProvider: llmProvider || 'OpenAI',
      model: model || 'GPT-4',
      fromChatAI: config?.fromChatAI || false
    };

    const explanationMessage = generateChatMessageForAgentSave(agentName, agentConfig);
    
    // Add the explanation message to chat
    const aiMessage = {
      id: Date.now() + 1,
    ...
    try {
      await supabase
        ...
        .insert({
          ...
          sender: 'ai',
          message_content: explanationMessage
        });
    } catch (error) {
      console.error('Error saving agent explanation to chat:', error);
    }
  ...
  const handleAgentDismiss = async (agentName: string) => {
    ...
    // Generate and send dismissal message
    const dismissalMessage = generateChatMessageForAgentDismissal(agentName);
    
    // Add the dismissal message to chat
    const aiMessage = {
      id: Date.now() + 1,
    ...
    // Save the dismissal message to the database
    try {
      await supabase
        .from('automation_chats')
        .insert({
          automation_id: automation?.id,
          sender: 'ai',
          message_content: dismissalMessage
        });
    } catch (error) {
      console.error('Error saving agent dismissal to chat:', error);
    }
