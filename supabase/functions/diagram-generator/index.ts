...
```json
{
    ...
    "labelStyle": { "fill": "color_for_label_text" },
    "labelBgStyle": { "fill": "color_for_label_background" }
}
```
...
serve(async (req) => {
    ...
    try {
        ...
        const messages = [
            ...
            {
                role: "user",
                // User message also emphasizes completeness and layout
        ...
        const response = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
                model: 'gpt-4o-2024-05-13', // Using the latest flagship model
                messages: messages,
        ...
        if (nodeCount < totalSteps * 0.7) { // Warn if less than 70% of steps are represented
            console.warn(`⚠️ Generated fewer nodes (${nodeCount}) than expected (${totalSteps}). Some steps may be missing in the diagram.`);
            // This is a server-side warning. The AI should now be doing better.
        }
